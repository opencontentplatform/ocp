...
Usage::
  ...
  $ python windowsCGClientService.py --startup delayed install
  $ python windowsCGClientService.py start
  $ python windowsCGClientService.py stop
  $ python windowsCGClientService.py remove
...
import utils
from contentGatheringClient import ContentGatheringClient
clientLabel = 'ITDM Content Gathering Client'


class ItdmClientService(win32serviceutil.ServiceFramework):
	_svc_name_ = 'ITDM_ContentGathering'
	...
	def spinUpTheClient(self, logger, settings, shutdownEvent, watcherPID):
		...
		thisClient = ContentGatheringClient(shutdownEvent, settings)
