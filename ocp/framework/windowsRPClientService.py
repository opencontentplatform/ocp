...
Usage::
  ...
  $ python windowsRPClientService.py --startup delayed install
  $ python windowsRPClientService.py start
...
import utils
from resultProcessingClient import ResultProcessingClient
clientLabel = 'ITDM Result Processing Client'


class ItdmClientService(win32serviceutil.ServiceFramework):
	_svc_name_ = 'ITDM_ResultsProcessing'
	...
	def spinUpTheClient(self, logger, settings, shutdownEvent, watcherPID):
		...
		thisClient = ResultProcessingClient(shutdownEvent, settings)
