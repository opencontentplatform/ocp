...
def convertCertificateTime(value):
...
def getSslCertificate(nameContext, port, connectTimeout=3):
	...
	with socket.create_connection((nameContext, port), connectTimeout) as sock:
		...
		with ctx.wrap_socket(sock, server_hostname=nameContext) as sslSocket:
			...
			cert = {
				...
				'version' : sslVersion,
				'not_before' : convertCertificateTime(stringify(x509cert.get_notBefore())),
				'not_after' : convertCertificateTime(stringify(x509cert.get_notAfter()))
			}
			...
			## end getSslCertificate
			return (sslVersion, cert)


def attemptSslHandshake(runtime, ipAddress, port, connectTimeout):
	"""Attempt an SSL handshake, returning the version and cert attributes."""
	version = None
	cert = None
	try:
		## Use IPs and ignore cert validation and name matching, to gather
		## self-signed certs
		nameContext = ipAddress
		runtime.logger.report(' trying address: {}:{}'.format(nameContext, port))
		(version, cert) = getSslCertificate(nameContext, port, connectTimeout)
		if len(cert) <= 0:
