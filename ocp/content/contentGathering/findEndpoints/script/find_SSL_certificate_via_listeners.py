...
def startJob(runtime):
	...
	try:
		...
		for port in runtime.endpoint.get('children', {}).get('TcpIpPort', []):
			...
			if continueFlag:
				(version, cert) = attemptConnection(runtime, ipAddress, portName, connectTimeout)
				if version is not None:
					## Create SSLCertificate with our previously created dict of attributes
					runtime.logger.report('Creating certificate on {}:{}'.format(ipAddress, portName))
